...
assert.ok(run('state.obstacles[0].y + state.obstacles[0].height > guido.y + 4'));
...
update(0.001);`);
assert.equal(run('state.obstacles.length'), 1);
...
state.bullets.push({x:498,y:275,width:10,height:4,vx:CONFIG.bulletSpeed,reflected:false});
update(0.001);`);
assert.equal(run('state.tubes.length'), 0);
...
run(`resetGame(); Math.random=()=>0.1; const beforeT=state.tubes.length; const beforeO=state.obstacles.length; spawnHazard(); globalThis.spawnDelta=(state.tubes.length-beforeT)+(state.obstacles.length-beforeO);`);
assert.equal(run('spawnDelta'), 1);
...
`);
assert.ok(run('CONFIG.fridgeChance <= 0.05'));
assert.ok(run('fridgeCount > 0 && fridgeCount < 45'));
assert.ok(run('fridgeMinObservedGap > CONFIG.fridgeMinSpawnSeparation'));
...
assert.equal(run('state.obstacles.length'), 0);
...
state.bullets.push({x:498,y:275,width:10,height:4,vx:CONFIG.bulletSpeed,reflected:false});
update(0.001);`);
assert.equal(run('state.bossAttacks.length'), 1);
assert.equal(run('state.bullets[0].reflected'), true);
assert.ok(run('state.bullets[0].vx < 0'));
